...
<a href="javascript:void(0)" class="d-flex align-items-center text-dark px-3 h-100 bg-white"
    style="border-left:2px solid var(--primary);" data-toggle="dropdown" data-display="static"
    title="{{translate('Cart')}}">
    <span style="margin-right: -17px;margin-left: 12px;">
        <svg xmlns="http://www.w3.org/2var(--primary)/svg" width="24" height="20.562" viewBox="0 0 24 20.562"
    ...
    <div>
        <span class="nav-box-text d-none d-xl-block ml-2 fs-12" style="color:var(--primary);margin-top: 10px;">
        ...
        <br>
        <span class="d-none d-xl-block ml-2 fs-14 fw-700"
            style="color:var(--primary);">{{ single_price($total) }}</span>
    </div>
</a>


<!-- Cart Items (Dropdown Sidebar) -->
<div class="dropdown-menu dropdown-menu-right dropdown-menu-lg p-0 stop-propagation rounded-0"
    style="top: -32vh; right: 0; height: 94vh; width: 300px; position: fixed; z-index: 9999; background-color: #fff;">
    @if (isset($carts) && count($carts) > 0)
        <div class="fs-16 fw-700 text-soft-dark pt-4 pb-2 mx-4 border-bottom" style="border-color: #e5e5e5 !important;">
            {{ translate('Cart Items') }}
        </div>
        <!-- Cart Products -->
        <ul class="h-360px overflow-auto c-scrollbar-light list-group list-group-flush mx-1" style="height: 64vh;">
            @foreach ($carts as $key => $cartItem)
                @php
                    $product = get_single_product($cartItem['product_id']);
                @endphp
                @if ($product != null)
                    <li class="list-group-item border-0 hov-scale-img d-flex flex-column justify-content-between">
                    ...
                    <div class="d-flex align-items-center justify-content-between mt-2"
                        style="border: 1px solid lightgray;width: 22vh;margin-left: 14vh;">
                        <!-- Minus Button -->
                        <button onclick="updateCartQuantity({{ $cartItem['id'] }}, 'decrease')" class="btn btn-sm btn-icon"
                            style="border-right: 1px solid lightgray;background-color: #d3d3d357;">
                            <i class="las la-minus fs-12"></i>
                        </button>

                        <!-- Quantity Display -->
                        <span id="cart-item-quantity-{{ $cartItem['id'] }}" class="mx-2">
                            {{ $cartItem['quantity'] }}
                        </span>

                        <!-- Plus Button -->
                        <button onclick="updateCartQuantity({{ $cartItem['id'] }}, 'increase')" class="btn btn-sm btn-icon"
                            style="border-right: 1px solid lightgray;background-color: #d3d3d357;">
        ...
        <div class="px-3 py-2 fs-15 border-top d-flex justify-content-between mx-4"
            ...
            <span id="cart-subtotal" class="fs-16 fw-700 text-dark">{{ single_price($total) }}</span>
        </div>
...
<script>
    function updateCartQuantity(cartItemId, action) {
        $.ajax({
            url: "{{ route('cart.update_quantity') }}", // Laravel route
            method: "POST",
            data: {
            ...
            success: function (response) {
                if (response.success) {
                    // Update Quantity
                    $("#cart-item-quantity-" + cartItemId).text(response.new_quantity);

                    // Update Total Price
                    $("#cart-subtotal").text(response.new_total_price);
                }
            }
        });
    }
